  ...
  return (
    <div id="evr">
        <form autocomplete="off" id="from" action='' method="POST" onSubmit={handleSubmit}>
            ...
            {!good3 && good1 && <span style={{margin:'10px 20px',display:'inline-block',color:'rgb(177, 7, 72)',border:'1px solid rgb(203, 184, 190)',padding:'15px 10px',backgroundColor:'#f8d7da',width:'90%'}}>enter your new password</span>}
...
{good3 && <div className="loader2"></div>}
  {!good3 && message && <p style={{    background:message.status==="error"? '#ff9090':'#5ded5d',
    margin: '10px 20px',
    color: 'white',
    borderRadius: '5px',
    padding: '10px',
    ...
    textAlign: 'center'}}>{message.status!=="error"?"Password Updated":"Invalid or expired token"}</p>}
